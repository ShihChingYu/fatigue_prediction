...
class ETLInputs(BaseModel):
    ...
    pvt_pattern: str = "pvt_*.csv"


class ETLOutputs(BaseModel):
    inputs_train: str = "data/processed/inputs_train.parquet"
    targets_train: str = "data/processed/targets_train.parquet"
    inputs_test: str = "data/processed/inputs_test.parquet"
    targets_test: str = "data/processed/targets_test.parquet"
    ids_train: str = "data/processed/ids_train.parquet"
    ids_test: str = "data/processed/ids_test.parquet"
...
class ETLJob(BaseModel):
    ...
    def __enter__(self):
        """Enter context: Setup logic (e.g. logging) goes here."""
        return self

    def __exit__(self, exc_type, exc_val, exc_tb):
        """Exit context: Teardown logic goes here."""
        pass

    def _get_user_ids(self, raw_path: Path):
        pattern_str = self.inputs.pvt_pattern
    ...
    def _engineer_hr(self, df):
        ...
        df["hr_jumpiness_5min"] = np.sqrt(
            (df["hr_diff"] ** 2).rolling("5min", min_periods=30).mean()
        )
    ...
    def _clean_pvt(self, file_path):
        ...
        # 3. Filter Artifacts (False starts)
        ...
        if "TAPTIME" in df.columns:
            ...
            possible_cols = ["RT_MEAN", "MEAN_RT", "PVT_MEAN_RT"]
            found = False
            for c in possible_cols:
                if c in df.columns:
                    ...
                    break
            if not found:
                raise KeyError("Could not find 'TAPTIME' or 'RT_MEAN' column.")
        # 5. Create Fatigue Label (0.0 to 1.0)
        df["fatigue_score"] = df["pvt_mean_rt"].apply(self._calculate_fatigue_score)
        df = df[df["timestamp"].dt.year > 2000]
        df = df.rename(columns={"timestamp": "TESTSTART"})  # rename timestamp to TESTSTART
    ...
    def _process_user(self, uid, raw_path):
        try:
            ...
            pvt_name = self.inputs.pvt_pattern.replace("*", uid)

            hr_path = raw_path / hr_name
            sleep_path = raw_path / sleep_name
            pvt_path = raw_path / pvt_name

            if not (hr_path.exists() and sleep_path.exists() and pvt_path.exists()):
                return None

            # 1. Generate Continuous Features (The Grid)
            ...
            # Impute Sleep (Improvement over your snippet: Don't drop, Fill!)
            ...
            # Resample to 1-min Grid (Matches your snippet)
            ...
            # 2. Process Targets (PVT)
            df_pvt = self._clean_pvt(pvt_path)

            # 3. Match Targets to Grid
            augmented_dfs = []
            ...
            for offset in offsets:
                ...
                # Merge PVT
                # use the 'timestamp' from PVT to find the feature vector
                merged = pd.merge_asof(
                    targets_shifted.sort_values("MATCH_TIME"),
                    df_continuous.sort_values("HRTIME"),
                    left_on="MATCH_TIME",
                    right_on="HRTIME",
                    direction="nearest",  # Matches snippet behavior (nearest grid point)
                    tolerance=pd.Timedelta(minutes=self.settings.pvt_tolerance_minutes),
                )
            ...
            # Final Clean
            required_cols = ["mean_hr_5min", "hours_awake", self.settings.target_col]
            df_final_user = df_final.dropna(subset=required_cols)
    ...
    def run(self):
        ...
        out_path = Path(self.outputs.inputs_train).parent
        ...
        log.info(f"Total labeled samples: {df_full.shape}")

        splitter = GroupShuffleSplit(
            n_splits=1, test_size=self.split.test_size, random_state=self.split.random_state
        )
        split_idx = splitter.split(df_full, groups=df_full["user_id"])
        train_idx, test_idx = next(split_idx)
        ...
        ids_train = train_df[["user_id", "HRTIME"]]
        ids_test = test_df[["user_id", "HRTIME"]]
        drop_cols = [
            target_col,
            "HR",
            "TESTSTART",
        ...
        # Save
        X_train.to_parquet(self.outputs.inputs_train, index=False)
        y_train.to_parquet(self.outputs.targets_train, index=False)
        ids_train.to_parquet(self.outputs.ids_train, index=False)

        X_test.to_parquet(self.outputs.inputs_test, index=False)
        y_test.to_parquet(self.outputs.targets_test, index=False)
        ids_test.to_parquet(self.outputs.ids_test, index=False)

        log.info(f"ETL Complete. Train: {X_train.shape}, Test: {X_test.shape}")
